...
# Leaves WSL MSI / Max-MSI alone. Never touches trading / FOMC / intraday processes.
#
# Usage (PowerShell as Maxwell, from C:\ cwd):
#   powershell -NoProfile -ExecutionPolicy Bypass -File .\park-windows-agent-workers.ps1
# Or from WSL:
#   /mnt/c/Windows/System32/WindowsPowerShell/v1.0/powershell.exe -NoProfile -ExecutionPolicy Bypass -File \\wsl$\Ubuntu-24.04\home\maxwell\Projects\active\max-msi-worker\park-windows-agent-workers.ps1
...
if ($null -eq $item) {
  "PARKED: Windows agent-cli disabled (better-sqlite3 ABI 127 vs 137). Use WSL MSI." |
...
Use WSL worker MSI instead:
  https://cursor.com/agents  -> environment MSI
  or Agents Window -> MSI (not Local / This computer / CODEX SPINE)
...
Write-Host "Done. Start tasks via MSI (WSL), not Windows Local." -ForegroundColor Green
exit 0
