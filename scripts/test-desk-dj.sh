...
grep -q 'Set-DeskPlaybackVolume' "$ROOT/scripts/desk-dj/Invoke-DeskDJ.ps1" && pass "volume auto-set helper" || fail "volume helper"
grep -q 'playback_volume' "$ROOT/scripts/desk-dj/playlist-config.json" && pass "playback_volume config" || fail "playback_volume"
grep -q '"playback_volume": 0.25' "$ROOT/scripts/desk-dj/playlist-config.json" && pass "default 25%" || fail "default 25%"
